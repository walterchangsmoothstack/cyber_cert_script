...
class SharepointUpload:
    ...
    def get_drive_id(self, site_name, drive_name):
        
        # Get the site ID of the site to upload to
        response = self.graph.get_site(site_name)
        ...
        response = self.graph.get_drive(site_id)
    ...
    def list_files_in_folder(self, drive_id, folder_path):
        # List the files in the target folder
        response = self.graph.list_files(drive_id, folder_path)
        json_data = response.json() if response and response.status_code == 200 else None
